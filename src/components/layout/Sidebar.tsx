...
function NavItemRow({ item }: { item: NavItem }) {
  return (
    ...
    >
      <span className="material-symbols-outlined text-[20px] transition-transform duration-200 group-hover:scale-110 shrink-0">
        {item.icon}
      </span>
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-[280px] flex flex-col z-50 glass-nav border-r border-white/[0.08]">
      ...
      <div className="px-3 pb-6 border-t border-white/[0.06] pt-3 shrink-0 space-y-0.5">
        ...
        {/* Logout — wired to Redux thunk */}
        <button
          onClick={handleLogout}
          className="flex items-center gap-3 px-4 py-[10px] rounded-xl text-sm text-red-400 hover:text-red-300 hover:bg-red-500/10 transition-all duration-200 w-full border-l-[3px] border-transparent font-medium"
        >
